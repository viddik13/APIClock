...
from wtforms import StringField, TextAreaField, BooleanField, SelectField,\
    SubmitField, TextField, PasswordField
...
class ChronoForm(playerForm):
    """Multi heritance test"""
    chrono_minutes = SelectField("Delay",
                                 validators=[Required("Choose a delay")])

    def __init__(self, *args, **kwargs):
        """Query for form list values."""
        super(ChronoForm, self).__init__(*args, **kwargs)

        chrono_mn = list(range(1, 60))
        self.chrono_minutes.choices = [(str(g), str(g)) for g in chrono_mn]
        self.chrono_minutes.choices.append((0, "Choose delay"))
